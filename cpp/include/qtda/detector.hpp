...
struct Signal {
  bool ready = false;      // full window available; betti is valid
  std::vector<int> betti;  // Betti-0 per eps (ascending eps order)
...
class StreamingDetector {
 public:
  ...
  Signal on_log_price(double x) {
    ...
    Signal sig;
    if (static_cast<int>(xbuf_.size()) < lag_ + 1) return sig;
    ...
    for (std::size_t i = 0; i < n; ++i) {
      for (std::size_t j = i + 1; j < n; ++j) {
        ...
        for (int k = 0; k < cfg_.m; ++k) {
          const double diff = points_[i][static_cast<std::size_t>(k)] - points_[j][static_cast<std::size_t>(k)];
          s += diff * diff;
        }
 ...
 private:
  void add_point(std::vector<double> p) {
    ...
    for (std::size_t i = 0; i < points_.size(); ++i) {
      double s = 0.0;
      for (int k = 0; k < cfg_.m; ++k) {
        const double diff = points_[i][static_cast<std::size_t>(k)] - p[static_cast<std::size_t>(k)];
        s += diff * diff;
      }
